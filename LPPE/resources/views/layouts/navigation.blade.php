<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if(auth()->user()->entraineur && auth()->user()->entraineur->rôle === 'admin')
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if(auth()->user()->entraineur && auth()->user()->entraineur->rôle === 'admin')
                    <x-responsive-nav-link :href="url('/planning')" :active="request()->is('planning')">
                        Séances
                    </x-responsive-nav-link>
                @endif
            @endauth
            <x-responsive-nav-link :href="route('entrainements.index')" :active="request()->routeIs('entrainements.*')">
                {{ __('Entraînements') }}
            </x-responsive-nav-link>
        </div>
